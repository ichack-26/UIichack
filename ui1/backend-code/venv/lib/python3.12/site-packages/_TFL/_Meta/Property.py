# -*- coding: utf-8 -*-
# Copyright (C) 2002-2025 Christian Tanzer. All rights reserved
...
#                       is left anymore
#    22-Sep-2025 (CT) Add `Typed_Property`
#                     + Factor `_Stored_Property_Mixin_`
#    ««revision-date»»···
#--
...
class _Stored_Property_Mixin_ (object) :
    """Mixin for properties which value is stored in the instance dict"""
    ...
    def __get__ (self, obj, cls = None) :
        if obj is None :
            return self._class_get (cls)
        return obj.__dict__  [self._s_name]
    # end def __get__
...
# end class _Stored_Property_Mixin_

class _Optional_Computed_Property_ (_Stored_Property_Mixin_) :
    """Property which value is computed unless explicitly set"""

    def __init__ (self, name, computer, compute_once = False, doc = None) :
        super ().__init__ (name, doc or computer.__doc__)
        self.computer     = self.__func__ = computer
...
class _Typed_Property_ (_Stored_Property_Mixin_) :
    """Property with value of specific type that is auto-converted"""

    def __init__ (self, name, type_caster, doc = None) :
        super ().__init__ (name, doc or type_caster.__doc__)
        self.type_caster  = self.__func__ = type_caster
    # end def __init__

    def __set__ (self, obj, value) :
        obj.__dict__ [self._s_name] = self.type_caster (value)
    # end def __set__

# end class _Typed_Property_

def Typed_Property (f) :
    """Decorator returning a `_Typed_Property_` with `f` as `type_caster`"""
    return _Typed_Property_ (f.__name__, f, f.__doc__)
### end def Typed_Property
