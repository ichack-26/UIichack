# -*- coding: utf-8 -*-
# Copyright (C) 2007-2024 Christian Tanzer. All rights reserved
...
#    15-Aug-2024 (CT) Add `average_angle`
#    16-Aug-2024 (CT) Normalize result of `average_angle` to `0..360`
#    ««revision-date»»···
#--
...
def average_angle (angles, weights = None) :
    """Returns the average value of the `angles`.

    >>> angles = (Angle_D (5), Angle_D (20), Angle_D (50))
    >>> print ("%.0f" % average_angle (angles))
    25

    >>> print ("%.0f" % average_angle ((-10, 20)))
    5

    >>> print ("%.0f" % average_angle ((-40, 20)))
    350

    >>> print ("%.0f" % average_angle ((0, 90), (10, 2)))
    11

    >>> print ("%.0f" % average_angle ((0, 90), (2, 10)))
    79

    """
    la  = len (angles)
    if la > 0 and not isinstance (angles [0], _Angle_) :
        angles  = tuple (Angle (a) for a in angles)
    if weights :
        lw  = len (weights)
        if la != lw :
            raise ValueError \
                ( "`angles` and `weights` must have equal length"
                  "; len (angles) == %d, len (weights) = %d" % (la, lw)
                )
        ew  = average (w * a.sin for a, w in zip (angles, weights))
        ns  = average (w * a.cos for a, w in zip (angles, weights))
    else :
        ew  = average (a.sin for a in angles)
        ns  = average (a.cos for a in angles)
    result  = (Angle_R (math.atan2 (ew, ns)).degrees + 360) % 360
    return result
# end def average_angle
