# -*- coding: utf-8 -*-
# Copyright (C) 2009-2025 Christian Tanzer All rights reserved
...
#     7-Aug-2024 (CT) Add `expect_msg` to `expect_except`
#                     + Allow for changed exception message in Python 3.13
#    12-Feb-2025 (CT) Add and use `_Spec_.auto_unquote`
#    15-Feb-2025 (CT) Add support for stdin to `CAO.__iter__`
#                     + Allow easy processing of pipes in place of arguments
#    16-Mar-2025 (CT) Add comment about using `sys.exit` to set exit status
#    ««revision-date»»···
#--
...
class _Spec_ (_Spec_Base_) :
    ...
    _unquoter     = Re_Replacer ( Regexp ("^(['" '"' "])(.*)\\1$"), "\\2")
...
class CAO (TFL.Meta.Object) :
    ...
    def __iter__ (self) :
        """Return an iterator over :attr:`argv` or :attr:`sys.stdin`."""
        argv = self.argv
...
def expect_except (* Xs, ** kw) :
    ...
    expect_msg = kw.pop ("expect_msg", None)
    try :
        yield
    except Xs as exc :
        ...
        if expect_msg is None :
            expect_msg = exc
        print ("%s: %s" % (exc.__class__.__name__, expect_msg))
...
If you want to set an explicit exit status, use::
