# -*- coding: utf-8 -*-
# Copyright (C) 1998-2025 Christian Tanzer. All rights reserved
...
@Single_Dispatch
def As_Euro (value) :
    """Return `value` as `Euro` instance if possible."""
    if isinstance (value, Euro) :
        return value
    if value is not None :
        raise ValueError \
            ( "Expected instance of `Euro`, got %r with value `%s`"
            % (type (value), value)
            )
# end def As_Euro

@As_Euro.add_type (int, float, decimal.Decimal, str)
def _As_Euro__from_number_or_str (value) :
    return EUR (value)
# end def _As_Euro__from_number_or_str
...
The function `As_Euro` will convert its argument to `Euro`, if possible:
