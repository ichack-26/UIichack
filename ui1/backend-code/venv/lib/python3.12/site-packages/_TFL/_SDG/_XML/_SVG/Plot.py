...
#    14-Mar-2024 (CT) Define `x_label_anchor` as attribute, not argument
#                     + Ditto for `y_label_anchor`
#    18-Mar-2024 (CT) Add `Grid.x_label_offset` and `.y_label_offset`
#    19-Mar-2024 (CT) Add `Grid.x_axis_label` and `.y_axis_label`
#    14-Apr-2024 (CT) Add `text_anchor` to `x_labels` arguments
#                     + Optional argument, default attribute
#                     + Ditto for `y_labels`
#    21-Apr-2024 (CT) Change symbols for `lines` of `x_ticks`, `y_ticks`
#                     * `|` for `x_ticks`, `-` for `y_ticks` aligns better
#                       with the direction of the grid lines specified
#                       (lines for `x_ticks` are vertical)
#    22-Apr-2024 (CT) Add `polymarker`
#    23-Apr-2024 (CT) Add optional argument `title` to `Plot`
#    24-Apr-2024 (CT) Change `Grid.[xy]_labels` to skip empty labels
#    24-Apr-2024 (CT) Change `Viewport` to skip empty transform
#    25-Apr-2024 (CT) Put ticks into `<g>`, add css classes
#    ««revision-date»»···
#--
...
from   _TFL.predicate             import filtered_join, pairwise, window_wise
...
class Grid (_Plot_Element_) :
    ...
    def x_axis_label \
        ...
        """Return a label to the x axis."""
        P       = self.P
        vp      = self.vp
        if dy is None :
            dy = DC.Y (3.5 * P.line_height)
        result = vp.group \
            ( vp.text
                ( label
                , dy           = dy
            ...
            , fill         = P.color.axis_text
            , font_size    = P.font_size * 1.75
            , klass        = klass
    ...
    def x_labels \
        ...
        P       = self.P
        vp      = self.vp
        xla     = kwds.pop ("text_anchor", self.x_label_anchor)
        result  = vp.group \
        ...
        for xi, x_label in zip (x_range, x_labels) :
            ...
            for xl in xls :
                if xl :
                    result.add \
                        (vp.text (xl, dx = dx + xlo, dy = dyi, x = xi, y = y))
                dyi += dyd
        return result
    # end def x_labels
    ...
    def x_ticks \
        ...
        lop     = tick_spec.line_opacity
        l_kwds  = dict \
            ( dict
                ( opacity      = lop if lop < 1 else None
        ...
        if "b" in sides :
            yield vp.group \
                ( * (   vp.line (xi, bot, xi, bot + tlen)
                    for xi in x_range
                    )
                , klass        = filtered_join (" ", ("x tck bot", kind))
                , ** t_kwds
                )
        if "t" in sides :
            yield vp.group \
                ( * (   vp.line (xi, top, xi, top - tlen)
                    for xi in x_range
                    )
                , klass        = filtered_join (" ", ("x tck top", kind))
                , ** t_kwds
                )
        if lines_p :
            yield vp.group \
                ( * (   vp.line (xi, bot + tlen, xi, top - tlen)
                    for xi in x_range
                    )
                , klass        = filtered_join (" ", ("x tck lin", kind))
                , ** l_kwds
                )
    # end def x_ticks
    ...
    def y_axis_label \
            ...
            , dx           = None
            , klass        = "y axs lab"
            , start_offset = NC.Y (0.5)
            , text_anchor  = "middle"
            ) :
        """Return a label to the y axis."""
        P       = self.P
        vp      = self.vp
        if dx is None :
            dx = - DC.X (4.0 * P.line_height)
        result = vp.group \
            ( vp.group
                ( vp.path
                    ( ( (NC.X (dx), NC.Y (1))
    ...
    def y_labels \
        ...
        P       = self.P
        vp      = self.vp
        yla     = kwds.pop ("text_anchor", self.y_label_anchor)
        result  = vp.group \
            ( fill         = P.color.axis_text if fill is None else fill
            , klass        = klass
            , stroke_width = stroke_width
        ...
        for yi, y_label in zip (y_range, y_labels) :
            ...
            for yl in yls :
                if yl :
                    result.add \
                        (vp.text (yl, dx = dx, dy = dyi + ylo, x = x, y = yi))
                dyi -= dyd
        return result
    # end def y_labels
    ...
    def y_ticks \
        ...
        if "l" in sides :
            yield vp.group \
                ( * (   vp.line (left, yi, left  + tlen, yi)
                    for yi in y_range
                    )
                , klass        = filtered_join (" ", ("y tck lft", kind))
                , ** t_kwds
                )
        if "r" in sides :
            yield vp.group \
                ( * (   vp.line (right, yi, right - tlen, yi)
                    for yi in y_range
                    )
                , klass        = filtered_join (" ", ("y tck rgt", kind))
                , ** t_kwds
                )
        if lines_p :
            yield vp.group \
                ( * (   vp.line (left + tlen, yi, right - tlen, yi)
                    for yi in y_range
                    )
                , klass        = filtered_join (" ", ("y tck lin", kind))
                , ** l_kwds
                )
    # end def y_ticks
    ...
    def _add_axis (self, ta, ticker, labeler, P, sides, svg) :
        if ta is not None :
            svg.add (* self._gen_ticks (ta, ticker, labeler, P, sides, svg))
            if ta.labels :
                svg.add \
                    ( labeler
    ...
    def _gen_ticks (self, ta, ticker, labeler, P, sides, svg) :
        if ta.major_range :
            yield from ticker \
                (ta.major_range,  P.major_ticks,  ta.major_lines,  sides, "maj")
        if ta.medium_range :
            yield from ticker \
                (ta.medium_range, P.medium_ticks, ta.medium_lines, sides, "med")
        if ta.minor_range :
            yield from ticker \
                (ta.minor_range,  P.minor_ticks,  ta.minor_lines,  sides, "min")
    # end def _gen_ticks
...
class Plot (_Plot_Element_) :
    ...
    def __init__ (self, width, height, P, title = None, ** kwds) :
        self.frame  = frame = Frame \
            ( x     = Frame_Axis (0, width)
            , y     = Frame_Axis (0, height)
            )
        self.P      = P
        self.svg    = self._create_svg (frame, ** kwds)
        self.title  = title
        if title is not None :
            self.add (SVG.Title (title))
    # end def __init__
...
class Viewport (_Plot_Element_) :
    ...
    def __init__ \
        ...
        self.svg           = self.group \
            ...
            , transform    = T.Translate (frame.x.offset, frame.y.offset)
                if frame.x.offset or frame.y.offset else None
            , ** kwds
            )
    ...
    def polymarker (self, mrk_id, points, ** kwds) :
        """Add marker `mrk_id` at `points` to the plot."""
        mrk     = "url(#%s)" % mrk_id
        kwds    = dict \
            ( dict
                ( marker_end    = mrk
        ...
        return self.polyline (points, ** kwds)
    # end def polyline
