...
MAJOR_HUBS = {
    ...
    "liverpool street", "bank", "leicester square", "green park", "paddington"
}
...
class JourneyScorer:
    ...
    def is_peak_hour(at: Optional[datetime] = None) -> bool:
        ...
        return ((h == 7 and m >= 30) or h == 8 or (h == 9 and m < 30) or (17 <= h < 19))

    @staticmethod
    def score_crowding(journey_data: Dict, preferences: Dict, disruptions: Dict[str, str], at: Optional[datetime] = None) -> tuple[int, str, str]:
        score = 100
        reasons = []
    ...
    def score_noise(journey_data: Dict, preferences: Dict) -> tuple[int, str, str]:
        ...
        for leg in journey_data.get("legs", []):
            ...
            if any(n in line for n in NOISY_LINES):
                score -= 15
                if "central" in line or "victoria" in line:
    ...
    @staticmethod
    def score_heat(journey_data: Dict, preferences: Dict, at: Optional[datetime] = None) -> tuple[int, str, str]:
        score = 100
        reasons = []
    ...
    @staticmethod
    def score_reliability(journey_data: Dict, preferences: Dict, disruptions: Dict[str, str]) -> tuple[int, str, str]:
        score = 100
        reasons = []
        legs = journey_data.get("legs", [])
        transport_legs = [l for l in legs if l.get("mode", {}).get("name") != "walking"]
        ...
        if bus_legs and preferences.get("prefer_buses"):
            score += 20
        elif not preferences.get("prefer_buses"):
            rail_legs = sum(1 for l in legs if l.get("mode", {}).get("name", "").lower() in RAIL_MODES)
            if rail_legs:
                score += 20  # reward trains if not preferring buses
    ...
    @staticmethod
    def calculate_overall_score(crowding_score: int, noise_score: int, heat_score: int,
                                reliability_score: int, preferences: Dict, journey_data: Dict) -> float:
        ...
        if preferences.get("prefer_buses"):
            ...
            overall -= bus_legs * 5  # penalize buses if not explicitly preferred

        overall = max(0, min(100, overall))
        return round(overall, 1)

    @staticmethod
    def generate_warnings(journey_data: Dict, disruptions: Dict[str, str],
                          crowding_score: int, heat_score: int, at: Optional[datetime] = None) -> List[str]:
        warnings = []
        for leg in journey_data.get("legs", []):
            line = (leg.get("routeOptions") or [{}])[0].get("name", "")
        ...
        for leg in journey_data.get("legs", []):
            for p in ("departurePoint", "arrivalPoint"):
                ...
                if name.lower() in MAJOR_HUBS:
                    hubs.add(name)
        if len(hubs) > 1:
            warnings.append(f"ℹ️ Route passes through major stations: {', '.join(list(hubs)[:2])}")
        return warnings
