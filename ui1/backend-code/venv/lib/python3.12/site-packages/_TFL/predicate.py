# -*- coding: utf-8 -*-
# Copyright (C) 1998-2024 Christian Tanzer. All rights reserved
...
#    23-Apr-2024 (CT) Add `dusplit_stable`
#    13-Nov-2024 (CT) Add `hashed_hex`
#    18-Nov-2024 (CT) Add `filter_zipped`
#    ««revision-date»»···
#--
...
def dusplit_stable (seq, decorator) :
    """`seq` split by `decorator` keeping original ordering of first instances.

       >>> dusplit_stable ([(0,1), (1,1), (0,2), (0,3), (2,3)], lambda x: x[1])
       [[(0, 1), (1, 1)], [(0, 2)], [(0, 3), (2, 3)]]

       >>> dusplit_stable ([(2,3), (0,3), (0,2), (1,1), (0,1)], lambda x: x[0])
       [[(2, 3)], [(0, 3), (0, 2), (0, 1)], [(1, 1)]]
    ...
    for (i, v) in enumerate (seq) :
        ...
        if k not in i_map :
            i_map [k] = i
    result  = list \
        (v_map [k] for k, i in sorted (i_map.items (), key = lambda x : x [1]))
    return result
# end def dusplit_stable
...
def filter_zipped (pred, * iterables) :
    """Filter slices of `iterables` for which at least one value fails `pred`.

    >>> filter_zipped ((lambda x : bool (x)), (1, 2, 3), ("a", "b", "c"))
    ((1, 2, 3), ('a', 'b', 'c'))

    >>> filter_zipped ((lambda x : bool (x)), (0, 1, 2), ("a", "b", ""))
    ((1,), ('b',))

    """
    def _gen () :
        for vs in zip (* iterables) :
            if all (pred (v) for v in vs) :
                yield vs
    return tuple (zip (* _gen ()))
# end def filter_zipped
...
def hashed_hex (x, length = None) :
    """Return sha1-value in hexdigest format.
    ...
    """
    hash    = hashlib.sha1   (pyk.encoded (x))
    dgst    = hash.hexdigest ()
    result  = dgst if length is None else dgst [:length]
    return result
# end def hashed_hex
