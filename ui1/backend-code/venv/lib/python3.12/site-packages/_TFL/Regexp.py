# -*- coding: utf-8 -*-
# Copyright (C) 2000-2025 Christian Tanzer. All rights reserved
...
#    15-Feb-2025 (CT) Add `__pattern` argument to `Dict_Replacer`
#                     + Allow caller-side caching of a complex pattern
#    22-Aug-2025 (CT) Add `re_esc_cleaner`
#                     For use in doctests to avoid warnings/errors from
#                     Python 3.14/3.15+ like::
#                         "\-" is an invalid escape sequence.
#                     when trying to `print` some output of `re.escape`
#    ««revision-date»»···
#--
...
class Dict_Replacer (Re_Replacer) :
    ...
    def __init__ \
            ( self, __dict = {}, __flags = 0, __head = "", __tail = ""
            , __pattern = None
            , ** kw
            ) :
        ...
        pattern   = "|".join \
            ...
            ) if __pattern is None else __pattern
        if __head or __tail :
...
### For use in doctests to avoid warnings/errors from Py3.14+ like::
#      "\-" is an invalid escape sequence.
### when trying to `print` some output of `re.escape`
re_esc_cleaner = Re_Replacer \
    ( r"\\([-!#$&*+.^|~])"
    , r"\1"
    )
